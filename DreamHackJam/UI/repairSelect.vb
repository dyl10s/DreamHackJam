                ...
                If ms.Y < (720 - 70) Then
                    ...
                    For Each t1 As turret1 In Game1.gameLoop.gameScene.turret1s
                        If New Rectangle(t1.realPos.X, t1.realPos.Y, t1.size.X, t1.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                            ...
                            If Globals.totalMoney >= Globals.t1Price / 2 Then

                                selected = False

                                For Each p As player In Game1.gameLoop.gameScene.players
                                    p.money -= Globals.t1Price / 2
                                Next
                    ...
                    For Each t2 As turret2 In Game1.gameLoop.gameScene.turret2s
                        If New Rectangle(t2.realPos.X, t2.realPos.Y, t2.size.X, t2.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                            ...
                            If Globals.totalMoney >= Globals.t2Price / 2 Then

                                selected = False

                                For Each p As player In Game1.gameLoop.gameScene.players
                                    p.money -= Globals.t2Price / 2
                                Next
                    ...
                    For Each t3 As turret3 In Game1.gameLoop.gameScene.turret3s
                        If New Rectangle(t3.realPos.X, t3.realPos.Y, t3.size.X, t3.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                            ...
                            If Globals.totalMoney >= Globals.t3price / 2 Then

                                selected = False

                                For Each p As player In Game1.gameLoop.gameScene.players
                                    p.money -= Globals.t3price / 2
                                Next
    ...
    Public Sub draw(sb As SpriteBatch)
        ...
        If selected = True Then
            If ms.Y < (720 - 70) Then
                ...
                Globals.cursorMode = "repair"

            End If
        Else

            If Globals.cursorMode = "repair" Then
        ...
        If selected = True Then
            For Each t1 As turret1 In Game1.gameLoop.gameScene.turret1s
                If New Rectangle(t1.realPos.X, t1.realPos.Y, t1.size.X, t1.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                    ...
                    sb.DrawString(Game1.contentMnger.loadFont("mainfont"), "$ " + (Globals.t1Price / 2).ToString, New Vector2(ms.X + 40, ms.Y), Color.Green, Nothing, Nothing, 1, Nothing, 0.8)
            ...
            For Each t2 As turret2 In Game1.gameLoop.gameScene.turret2s
                If New Rectangle(t2.realPos.X, t2.realPos.Y, t2.size.X, t2.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                    ...
                    sb.DrawString(Game1.contentMnger.loadFont("mainfont"), "$ " + (Globals.t2Price / 2).ToString, New Vector2(ms.X + 40, ms.Y), Color.Green, Nothing, Nothing, 1, Nothing, 0.8)
            ...
            For Each t3 As turret3 In Game1.gameLoop.gameScene.turret3s
                If New Rectangle(t3.realPos.X, t3.realPos.Y, t3.size.X, t3.size.Y).Contains(New Rectangle(ms.X - Globals.cameraLoc.X, ms.Y - Globals.cameraLoc.Y, 1, 1)) Then
                    ...
                    sb.DrawString(Game1.contentMnger.loadFont("mainfont"), "$ " + (Globals.t3price / 2).ToString, New Vector2(ms.X + 40, ms.Y), Color.Green, Nothing, Nothing, 1, Nothing, 0.8)
